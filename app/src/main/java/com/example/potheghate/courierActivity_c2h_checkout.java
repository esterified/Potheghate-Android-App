...
import com.example.potheghate.Adapter.c2hCheckoutAdapter;
...
public class courierActivity_c2h_checkout extends AppCompatActivity {
    ...
    public static final String COURIER_C_2_H = "courier_c2h";
    ...
    public ArrayList<Float> price_list = new ArrayList<>();
    private static final String ORDER="order";
    public TextView totalPrice;
    private c2hCheckoutAdapter adapterMain;
    private progressBar progBar;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_courier_c2h_checkout);
    ...
    private void submitData() {
        ...
        ref.child(COURIER_C_2_H).child(user.getUid()).addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()) {
                    String key = Objects.requireNonNull(ref.child(ORDER).push().getKey());
                    ...
                    ref.child(ORDER).child(user.getUid()).child(key).child(TYPE).setValue("c2h");
                    ...
                    ref.child(COURIER_C_2_H).child(user.getUid()).setValue(null).addOnCompleteListener(new OnCompleteListener<Void>() {
                        @Override
        ...
        ref.child(COURIER_C_2_H).child(user.getUid()).addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()) {
                    ...
                    adapterMain = new c2hCheckoutAdapter(data, data_from, data_to, price_list,price_all,price_all_courier);
                    RecyclerView r = findViewById(R.id.checkoutRecycleView);
                    r.setAdapter(adapterMain);
                    setTotalPrice(getTotalPrice(price_list));
                    adapterMain.SetOnRemoveListener(new c2hCheckoutAdapter.OnRemoveListener() {
                        @Override
    ...
    private void setDatalistener() {
        FirebaseUser user = Objects.requireNonNull(FirebaseAuth.getInstance().getCurrentUser());
        DatabaseReference ref = FirebaseDatabase.getInstance().getReference();
        ref.child(COURIER_C_2_H).child(user.getUid()).addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
...
private void goback(){
    startActivity(new Intent(getApplicationContext(), courierActivity_c2h.class).setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_NEW_TASK));
