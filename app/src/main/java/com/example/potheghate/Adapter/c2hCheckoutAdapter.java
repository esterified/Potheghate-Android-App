...
public class c2hCheckoutAdapter extends RecyclerView.Adapter<c2hCheckoutAdapter.viewHolder> {
    ...
    private c2hCheckoutAdapter.OnRemoveListener listener;

    public c2hCheckoutAdapter(ArrayList<String> data, HashMap<String, ArrayList<c_data_from>> data_from, HashMap<String, ArrayList<c_data_to>> data_to, ArrayList<Float> price_list,
                              HashMap<String, ArrayList<c_price>> price_all,HashMap<String, ArrayList<c_price>> price_all_courier) {
        this.data = data;
    ...
    public void SetOnRemoveListener(c2hCheckoutAdapter.OnRemoveListener listener) {
        this.listener = listener;
    }

    @NonNull
    @Override
    public c2hCheckoutAdapter.viewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.courier_h2c_checkout_list, parent, false);
        return new c2hCheckoutAdapter.viewHolder(view, listener);
    }

    @Override
    public void onBindViewHolder(@NonNull c2hCheckoutAdapter.viewHolder holder, int position) {
        ...
        c2hChildFromAdapter adapter = new c2hChildFromAdapter(data_from.get(data.get(position)),price_all,price_all_courier);
        holder.getViewToR().setAdapter(adapter);
        c2hChildToAdapter adapterT = new c2hChildToAdapter(data_to.get(data.get(position)),price_all,price_all_courier,data_from.get(data.get(position)));
        holder.getViewFromR().setAdapter(adapterT);
    }
    ...
    public static class viewHolder extends RecyclerView.ViewHolder {
        ...
        public viewHolder(@NonNull View itemView, c2hCheckoutAdapter.OnRemoveListener listener) {
            super(itemView);
